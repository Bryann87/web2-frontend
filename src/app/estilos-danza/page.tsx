...
export default function EstilosDanzaPage() {
  ...
  const handleDelete = async (estilo: EstiloDanza) => {
    if (window.confirm('¿Estás seguro de eliminar este estilo de danza?')) {
      try {
        await estilosDanzaService.delete(estilo.idEstilo);
        refetch();
      } catch (error) {
        alert('Error al eliminar el estilo de danza');
      }
    }
  };
