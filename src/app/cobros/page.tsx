...
const metodosPago = PAYMENT_METHODS;
...
export default function CobrosPage() {
  ...
  const onSubmit = async (data: CobroForm) => {
    setIsLoading(true);
    try {
      const mesCorrespondiente = data.mesCorrespondiente || '';
                ...
                render={({ field }) => (
                  <SearchableSelect
                    ...
                    options={mesesOptions.map((mes) => ({
                      value: `${mes} ${new Date().getFullYear()}`,
                      label: `${mes} ${new Date().getFullYear()}`,
                    }))}
                    value={field.value || ''}
                        ...
                        <tbody>
                          {historialPagos.map((cobro) => (
                            <tr key={cobro.idCobro} className="border-b">
                              ...
                              <td className="py-2 px-2">{cobro.metodoPago}</td>
